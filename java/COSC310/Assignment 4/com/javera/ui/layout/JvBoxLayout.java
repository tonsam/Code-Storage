 ...
 */
public class JvBoxLayout implements LayoutManager2, LayoutPrintLayout, Serializable {
	/**
	...
	public int layoutPrint(Container target, int pageHeight) {
		...
		for (int i = 0; i < target.getComponents().length; i++) {
			...
			if (child.isVisible()) {
				...
				if (child instanceof LayoutPrint) {
					childHeight = ((LayoutPrint) child).layoutPrint(pageHeight);
				} else {
