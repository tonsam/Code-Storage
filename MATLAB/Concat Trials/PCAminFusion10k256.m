...
[imdsTrainA,imdsTestA] = splitEachLabel(imdsTrimmed,0.5,'randomized');

imdsTrimmed2.ReadFcn = @(filename)readAndPreprocessImageV(filename);
[imdsTrainV,imdsTestV] = splitEachLabel(imdsTrimmed2,0.5,'randomized');
...
featuresFused = min(featuresTrainA, featuresTrainB);
featuresFused = min(featuresFused, featuresTrainC);
featuresFused = rot90(pca(featuresFused));
...
featuresTestFused = min(featuresTestA, featuresTestV1);
featuresTestFused = min(featuresTestFused, featuresTestV2);
featuresTestFused = pca(featuresTestFused);
...
classifier = fitcecoc(featuresFused,YTrain,'Learners', 'Linear',...
