...
featuresTrain = activations(netTransfer,imdsTrain,layer,...
    'MiniBatchSize', 1,'OutputAs','rows',...
    'ExecutionEnvironment','gpu');

featuresTest = activations(netTransfer,imdsValidation,layer,...
    'MiniBatchSize', 1,'OutputAs','rows',...
    'ExecutionEnvironment','gpu');

% apply PCA
vis1 = pca(featuresTrain);
vis2 = pca(featuresTest);

figure('Name','Transfer VGG16 Training Features');
surf(vis1,'EdgeColor','none');
figure('Name','Transfer VGG16 Test Features');
surf(vis2,'EdgeColor','none');
